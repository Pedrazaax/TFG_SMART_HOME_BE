...
@app.get("/getlogs")
async def get_logs(user: User = Depends(current_user)):
    ...
    try:
        logs = await historylogsService.get_logs_from_ha(token, dominio)
        print(logs)  # Imprime la respuesta de los logs
        return logs
...
@app.get("/gethistory/{timestamp}")
async def get_history(timestamp: str, user: User = Depends(current_user)):
    # Verificar si el usuario está autenticado a través del token JWT en la cabecera
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized")
    ...
    try:
        history = await historylogsService.get_history_from_ha(token, dominio, timestamp)
        return history
    except Exception as e:
